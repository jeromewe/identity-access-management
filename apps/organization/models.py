...
class Organization(models.Model):
    ...
    users = models.ManyToManyField('account.User', related_name='related_user_orgs', blank=True)
    admins = models.ManyToManyField('account.User', related_name='related_admin_orgs', blank=True)
    auditors = models.ManyToManyField('account.User', related_name='related_audit_orgs', blank=True)
    created_by = models.CharField(max_length=32, null=True, blank=True, verbose_name=_('Created by'))
    date_created = models.DateTimeField(auto_now_add=True, null=True, blank=True, verbose_name=_('Date created'))
    comment = models.TextField(max_length=128, default='', blank=True, verbose_name=_('Comment'))

    orgs = None
    CACHE_PREFIX = 'L2C_ORG_{}'
    ...
    def get_user_admin_or_audit_orgs(cls, user):
        admin_orgs = cls.get_user_admin_orgs(user)
        audit_orgs = cls.get_user_audit_orgs(user)
        orgs = set(admin_orgs) | set(audit_orgs)
        return orgs

    @classmethod
    def get_user_or_user_orgs_or_audit_orgs_or_admin_orgs(cls, user):
        admin_orgs = cls.get_admin_orgs(user)
        audit_orgs = cls.get_auduit_orgs(user)
        user_orgs = cls.get_user_user_orgs(user)
        orgs = set(admin_orgs) | set(audit_orgs) | set(user_orgs)
        return orgs
