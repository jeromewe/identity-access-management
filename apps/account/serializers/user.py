...
class RegisterAccountSerializer(serializers.Serializer):
    company = serializers.CharField(
        label=_('公司名称'), required=True,
        max_length=128, min_length=1
    )
    name = serializers.CharField(
        label=_('名称'), required=False,
    )
    phone = serializers.CharField(
        label=_('手机号码'), required=False,
        max_length=11, min_length=11
    )
    email = serializers.EmailField(label=_('电子邮件地址'), required=False)
    password = serializers.CharField(
        label=_('密码'), write_only=True, max_length=128, required=True
    )
    terms_of_service = serializers.BooleanField(label=_('服务条款'), required=True)
    privacy_policy = serializers.BooleanField(label=_('隐私条款'), required=True)
    ...
    def validate_email(email):
        if User.objects.filter(email=email):
            raise serializers.ValidationError("电子邮件地址已存在")
        return email
    ...
    def validate_company(company):
        ...
        if Organization.objects.filter(name=company):
            raise serializers.ValidationError("公司名称不合法")
        return company
